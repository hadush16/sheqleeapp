...
export default function FreelancerProfile() {
  ...
  const handleAddLinkFromModal = (link) => {
    setForm((p) => ({ ...p, profiles: [...p.profiles, link] }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        err.response?.data?.message || "Failed to update freelancer profile",
      );
    } finally {
      setIsLoading(false);
    }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end mb-4 mt-10">
            <button
              type="submit"
              disabled={isLoading}
              className={`bg-[#8967B3] text-white text-[18px] md:text-[26px]
    px-4 py-3 md:px-5 md:py-4 rounded-lg font-semibold
  ...
  `}
            >
              {isLoading ? (
                <>
                  <span className="w-5 h-5 border-2 border-white border-t-blue-400 rounded-full animate-spin" />
                  Updating...
                </>
              ) : (
                "Update profile"
