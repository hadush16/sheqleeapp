...
// export default function AccountSettingFreelancer() {
...
//     if (!PASSWORD_UNCHANGED && !passwordsMatch) return;
...
//     console.log("DELETE FREELANCER ACCOUNT:", form.deleteReason);
//   };
...
//                 className="w-full mt-2 min-h-[120px] bg-[#E5E5E5] rounded-lg p-3 text-sm outline-none resize-none"
//               />
...
//         const freelancer = JSON.parse(raw);

//         const res = await api.get(`/freelancers/${freelancer._id}`);

//         setForm((p) => ({
//           ...p,
//           fullName: res.data.data.freelancer.fullName || "",
//           email: res.data.data.freelancer.email || "",
//         }));

//         setFreelancerId(freelancer._id);
//       } catch (err) {
//         console.error(err);
//         alert("Failed to load freelancer account");
//       } finally {
//         setLoading(false);
//       }
//     };
...
//       await api.patch(`/freelancers/${freelancerId}`, {
...
//       await api.delete(`/freelancers/${freelancerId}`, {
...
//                 Full name <span className="text-[#FF2626]">*</span>
//               </label>
...
//           <hr className="w-full h-[3px] bg-[#DFDFDF] border-0 my-6" />

//           {/* DELETE */}
//           <div>
//             <h2 className="text-[22px] font-semibold mb-3">Account deletion</h2>

//             <p className="text-[16px] lg:text-[23px] mb-8">
...
//               }
//               placeholder="Why are you deleting your account..."
//               className="w-full min-h-[200px] bg-[#DFDFDF] rounded-lg p-4 text-[16px] lg:text-[23px] outline-none resize-none mt-2"
//             />
//              {errors.deleteReason && (
...
//               <button
//                 onClick={handleDelete}
//                 className="bg-red-500 text-white px-4 py-3 rounded-[11px] text-[27px] font-semibold mb-2"
//               >
...
//             "Failed to load freelancer account",
//         });
//       } finally {
//         setLoading(false);
//       }
...
//   const inputClass =
//     "flex-1 px-3 bg-[#DFDFDF] outline-none border-none text-[16px] lg:text-[23px]";
...
//         <div className="text-center mb-12">
//           <img src={settingsIcon} className="w-14 h-14 mx-auto mb-4" />
//           <h1 className="text-[23px] lg:text-[45px] font-semibold">
//             Account Setting
//           </h1>
//         </div>
...
//             <div className="mt-2 flex items-center bg-[#DFDFDF] rounded-lg">
//               <input
//                 ref={nameRef}
//                 value={form.fullName}
//                 onChange={(e) =>
...
//                 className={inputClass}
//               />
//               <button onClick={() => nameRef.current?.focus()} className="px-3">
//                 <img src={blackIconedit} className="w-4 h-4 opacity-70" />
//               </button>
//             </div>
//             {errors.fullName && <p className={errorText}>{errors.fullName}</p>}
...
//                 className={inputClass}
//               />
//               <button onClick={() => emailRef.current?.focus()} className="px-3">
//                 <img src={blackIconedit} className="w-4 h-4 opacity-70" />
//               </button>
//             </div>
//             {errors.email && <p className={errorText}>{errors.email}</p>}
...
//                 setForm((p) => ({ ...p, password: e.target.value }))
//               }
//               className={`${inputClass} w-full rounded-lg`}
//             />
//             {errors.password && <p className={errorText}>{errors.password}</p>}
//           </div>
...
//             onClick={handleUpdate}
//             disabled={submittingUpdate}
//             className="bg-[#8967B3] text-white px-6 py-3 rounded-[11px] text-xl font-semibold disabled:opacity-50"
//           >
//             {submittingUpdate ? "Updating..." : "Update setting"}
//           </button>
//         </div>
...
//               setForm((p) => ({ ...p, deleteReason: e.target.value }))
//             }
//             className="w-full min-h-[160px] bg-[#DFDFDF] rounded-lg p-4"
//           />
//           {errors.deleteReason && (
//             <p className={errorText}>{errors.deleteReason}</p>
//           )}
...
//               className="bg-red-500 text-white px-6 py-3 rounded-[11px] text-xl font-semibold disabled:opacity-50"
//             >
//               {submittingDelete ? "Deleting..." : "Delete account"}
//             </button>
//           </div>
  ...
  useEffect(() => {
    const fetchFreelancer = async () => {
      try {
        ...
        setErrors({
          global: "Failed to load freelancer account",
        });
      } finally {
        setLoading(false);
      }
  ...
  return (
    <>
      ...
      <section className="px-4 sm:px-6 py-12 max-w-[1000px] mx-auto">
        ...
        <div className="text-center mb-12">
          <img src={settingsIcon} className="w-14 h-14 mx-auto mb-4" />
          <h1 className="text-[23px] lg:text-[45px] font-semibold mb-4">
            Account Setting
          </h1>
          <p className="text-[23px] lg:text-[27px] max-w-[600px] mx-auto">
            Edit your account settings to make your account more secure.
          </p>
        </div>
        ...
        <div className="space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="mt-2 flex items-center bg-[#DFDFDF] rounded-lg px-1 py-4">
                <input
                  ref={nameRef}
                  value={form.fullName}
                  onChange={(e) =>
          ...
          <div className="flex justify-end">
            <button
              onClick={handleUpdate}
              disabled={submittingUpdate}
              className="bg-[#8967B3] text-white px-6 py-3 rounded-[11px] text-[22px] font-semibold"
            >
              {submittingUpdate ? "Updating..." : "Update setting"}
            </button>
          </div>

          <hr className="h-[3px] bg-[#DFDFDF] border-0" />
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full min-h-[200px] bg-[#DFDFDF] rounded-lg p-4 mt-2"
            />
            {errors.deleteReason && (
              <p className={errorText}>{errors.deleteReason}</p>
            )}
            ...
            <div className="flex justify-end mt-6">
              <button
                onClick={handleDelete}
                disabled={submittingDelete}
                className="bg-red-500 text-white px-6 py-3 rounded-[11px] text-[22px] font-semibold"
              >
                {submittingDelete ? "Deleting..." : "Delete account"}
              </button>
            </div>
